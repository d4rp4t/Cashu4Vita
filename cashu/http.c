...
// ====================================================================
// sce_http was dropped because its SSL stack doesn't support TLS 1.2
// replaced with libcurl + mbedTLS.
// ====================================================================
...
cashu_err_t cashu_http_init(void) {
    ...
    p.flags  = 0;
    sceNetInit(&p);   /* ignore — may already be up */
    sceNetCtlInit();  /* ignore */

    if (curl_global_init(CURL_GLOBAL_DEFAULT) != CURLE_OK) {
        sceNetCtlTerm(); sceNetTerm();
        free(s_net_mem); s_net_mem = NULL;
...
cashu_err_t cashu_get_keys(const char *mint_url, keyset_t **out, size_t *count) {
    char *url = make_url(mint_url, "keys"); if (!url) return CASHU_ERR_OOM;
    char *resp = NULL;
    cashu_err_t err = do_request(M_GET, url, NULL, &resp); free(url);
    if (err != CASHU_OK) return err;
    err = json_parse_keys(resp, out, count); free(resp); return err;
}

cashu_err_t cashu_get_keysets(const char *mint_url, keyset_t **out, size_t *count) {
    char *url = make_url(mint_url, "keysets"); if (!url) return CASHU_ERR_OOM;
    char *resp = NULL;
    cashu_err_t err = do_request(M_GET, url, NULL, &resp); free(url);
    if (err != CASHU_OK) return err;
    err = json_parse_keysets(resp, out, count); free(resp); return err;
}

cashu_err_t cashu_mint_quote(const char *mint_url, uint64_t amount,
                             const char *unit, mint_quote_t *out) {
    char *url = make_url(mint_url, "mint/quote/bolt11"); if (!url) return CASHU_ERR_OOM;
    char *body = json_mint_quote_request(amount, unit);
    if (!body) { free(url); return CASHU_ERR_OOM; }
    char *resp = NULL;
    cashu_err_t err = do_request(M_POST, url, body, &resp); free(url); free(body);
    if (err != CASHU_OK) return err;
    err = json_parse_mint_quote(resp, out); free(resp); return err;
}
...
cashu_err_t cashu_mint_quote_state(const char *mint_url, const char *quote_id,
    ...
    char *url = make_url(mint_url, path); if (!url) return CASHU_ERR_OOM;
    char *resp = NULL;
    cashu_err_t err = do_request(M_GET, url, NULL, &resp); free(url);
    if (err != CASHU_OK) return err;
    err = json_parse_mint_quote(resp, out); free(resp); return err;
...
cashu_err_t cashu_mint(const char *mint_url, const char *quote,
    ...
    char *url = make_url(mint_url, "mint/bolt11"); if (!url) return CASHU_ERR_OOM;
    char *body = json_mint_request(quote, outputs, count);
    if (!body) { free(url); return CASHU_ERR_OOM; }
    char *resp = NULL;
    cashu_err_t err = do_request(M_POST, url, body, &resp); free(url); free(body);
    if (err != CASHU_OK) return err;
    err = json_parse_signatures(resp, sigs_out, sig_count); free(resp); return err;
}

cashu_err_t cashu_melt_quote(const char *mint_url, const char *bolt11,
                             const char *unit, melt_quote_t *out) {
    char *url = make_url(mint_url, "melt/quote/bolt11"); if (!url) return CASHU_ERR_OOM;
    char *body = json_melt_quote_request(bolt11, unit);
    if (!body) { free(url); return CASHU_ERR_OOM; }
    char *resp = NULL;
    cashu_err_t err = do_request(M_POST, url, body, &resp); free(url); free(body);
    if (err != CASHU_OK) return err;
...
cashu_err_t cashu_swap(const char *mint_url,
                       ...
                       const blinded_message_t *outputs, size_t out_n,
                       blind_signature_t **sigs_out, size_t *sig_count) {
    char *url = make_url(mint_url, "swap"); if (!url) return CASHU_ERR_OOM;
    char *body = json_swap_request(inputs, inp_n, outputs, out_n);
    if (!body) { free(url); return CASHU_ERR_OOM; }
    char *resp = NULL;
    cashu_err_t err = do_request(M_POST, url, body, &resp); free(url); free(body);
    if (err != CASHU_OK) return err;
